...
ACUTILS_SYMBOL_ATTRIBUTES PRIVATE_ACUTILS_ST_FUNC void* private_ACUtilsTest_DynArray_realloc(void* ptr, size_t size) {
...
ACUTILS_SYMBOL_ATTRIBUTES PRIVATE_ACUTILS_ST_FUNC void private_ACUtilsTest_DynArray_free(void* ptr) {
    if(ptr != NULL) {
...
static size_t calculateCapacityTest(size_t requiredSize) {
    return private_ACUtils_DynArray_calculateCapacityGeneric(
...
START_TEST(test_aDynArrayConstruct_aDynArrayDestruct_valid)
{
    ...
    array = aDynArrayConstructWithAllocator(struct DynStringTestArray, private_ACUtilsTest_DynArray_realloc, private_ACUtilsTest_DynArray_free);
